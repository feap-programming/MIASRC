...
Public Class FldAssemblyDowntimeMain
    ...
    Private Sub cbbDownTime_Click(sender As Object, e As EventArgs) Handles cbbDownTime.Click
        ...
        Dim type As String = gvData.CurrentRow.Cells("fldType").Value.ToString
        Dim key = $"DownTime_{type}".ToUpper()
        Dim dlineValue = gvData.CurrentRow.Cells("fldLine").Value.ToString()
        Dim finalLine As String = If(globalVariables.DowntimeType = "Taping",
                                dlineValue.Replace("L", ""),
                                dlineValue)
        FormManager.ShowForm(Of FrmAssemblyDownTime)(key,
            Sub(f)
                f.DowntimeType = type
                f.AssyDownTimeID = gvData.CurrentRow.Cells("fldID").Value
                f.AssyDownTimeModel = gvData.CurrentRow.Cells("fldModel").Value.ToString()
                f.AssyDownTimeLine = finalLine
                f.AssyDownTimeDate = gvData.CurrentRow.Cells("fldDate").Value.ToString()
                f.AssyDownTimeType = type
                f.FWGroupDt = gvData.CurrentRow.Cells("fldFwGroup").Value.ToString()
                f.Text = $"{FormManager.GetFormTitle(type)} Downtime"
            End Sub)
        Me.Close()
    End Sub
