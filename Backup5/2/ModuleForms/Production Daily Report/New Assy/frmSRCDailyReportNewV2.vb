...
Public Class FrmSRCDailyReportNewV2
    ...
    Private Sub FrmSRCDailyReportNewV2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        readycbModelName()
        'readyMcbModelName()
        lblOptimeText.Visible = True
        lblTotalOperationTime.Visible = True
        ...
        assyLineStatus = "ADD"
        cbManpower.SelectedIndex = -1
        ddStatus.Visible = False
        rcbQualified.Enabled = False
        rcbTransfer.Enabled = False
        rcbTrainee.Enabled = False
    ...
    Private Sub GetDownTime()
        ...
        Dim DTDownTimeCheckPlan As DataTable = Furina.SPSelect("SP_SRCDownTime_SelectByDailyReportv2")
        ...
        Furina.AddMySqlParameters("parType", "ASSY")
        Furina.AddMySqlParameters("parLine", Mio.Line)
        Dim DTDownTimeCheckUnplan As DataTable = Furina.SPSelect("SP_SRCDownTime_SelectByDailyReportv2")
    ...
    Private Sub ComputeCycleTime()
        Try
            ...
            For rowcounter As Integer = 0 To Me.rgvAssyLineEff.Rows.Count - 1
                ...
                Dim timePartition As String = rgvAssyLineEff.Rows(rowcounter).Cells("fldTimePartition").Value.ToString()
                ...
                End If
                lblTotalOperationTime.Text = sumOperationTime
            ...
            Dim CycleTime As Decimal = ((sumOperationTime - (HottoVal + PlanedDTVal + UnplanedDTVal)) * 60) / sum
...
#Region "Manpower"
    ...
    Private Sub GetTL()
        ...
        Furina.AddMySqlParameters("parLine", Mio.Line)
        Dim DTTeamLeader As DataTable = Furina.SPSelect("SP_SelectTeamLeaderv3")
        ...
        Dim fldQualified As New GridViewTextBoxColumn("Qualified")
        fldQualified.FieldName = "fldQualified"
        fldQualified.Width = 200
        fldQualified.IsVisible = False
        cbManpower.MultiColumnComboBoxElement.Columns.Add(fldQualified)
    ...
    Private Sub GetTeam()
        ...
        If ddStatus.Text = "Qualified" Then
            rcbQualified.CheckState = CheckState.Checked
        ElseIf ddStatus.Text = "Transferred" Then
            rcbTransfer.CheckState = CheckState.Checked
        ElseIf ddStatus.Text = "Trainee" Then
            rcbTrainee.CheckState = CheckState.Checked
        End If
    ...
    Private Sub btnSubmit_Click(sender As Object, e As EventArgs) Handles btnMPSubmit.Click
        ...
        Else
            ...
            '    parTrainee = 0
            'End If
            If rcbQualified.CheckState = CheckState.Checked Then
                parQualify = 1
            ElseIf rcbTransfer.CheckState = CheckState.Checked Then
                parTransfer = 1
            ElseIf rcbTrainee.CheckState = CheckState.Checked Then
                parTrainee = 1
            Else
                parQualify = 0
                parTransfer = 0
    ...
    Private Sub rgvManpower_DoubleClick(sender As Object, e As EventArgs) Handles rgvManpower.DoubleClick
        ...
        rcbQualified.Enabled = True
        rcbTransfer.Enabled = True
        rcbTrainee.Enabled = True
        ddProc.Enabled = True
        ...
        If rgvManpower.CurrentRow.Cells("fldQualified").Value = 1 Then
            rcbQualified.CheckState = CheckState.Checked
        Else
            rcbQualified.CheckState = CheckState.Unchecked
        End If
        If rgvManpower.CurrentRow.Cells("fldTransferred").Value = 1 Then
            rcbTransfer.CheckState = CheckState.Checked
        Else
            rcbTransfer.CheckState = CheckState.Unchecked
        End If
        If rgvManpower.CurrentRow.Cells("fldTrainee").Value = 1 Then
            rcbTrainee.CheckState = CheckState.Checked
        Else
            rcbTrainee.CheckState = CheckState.Unchecked
        End If
...
#Region "Hourly"
    ...
    Private Sub GetHourlyShiftTime()
        ...
        ' Always use consistent interval logic regardless of ToMin
        ' 1. Add initial partial hour if needed
        If FromTime.Minute <> 0 Then
            Dim nextHour As DateTime = New DateTime(FromTime.Year, FromTime.Month, FromTime.Day, FromTime.Hour, 0, 0).AddHours(1)
            TimeRow.Rows.Add(FromTime.ToString("HH:mm"))
            FromTime = nextHour
        End If

        ' 2. Add full hour markers
        While FromTime < ToTime
            TimeRow.Rows.Add(FromTime.ToString("HH:mm"))
            FromTime = FromTime.AddHours(1)
        End While

        ' 3. Add final mark (ToTime)
        If TimeRow.Rows(TimeRow.Rows.Count - 1).Item(0).ToString() <> ToTime.ToString("HH:mm") Then
            TimeRow.Rows.Add(ToTime.ToString("HH:mm"))
        End If
    ...
    Private Sub btnHQSubmit_Click(sender As Object, e As EventArgs) Handles btnHQSubmit.Click
        ...
        Dim Rotator As String = spnHQRotator.Text
        Dim Stator As String = spnHQStator.Text
        Dim SubStator As String = spnHQSubstator.Text
        Dim Sleeve As String = spnHQSleeve.Text
    ...
    Private Sub rgvHourly_CellDoubleClick(sender As Object, e As GridViewCellEventArgs) Handles rgvHourly.CellDoubleClick
        ...
        spnHQRotator.Text = rgvHourly.CurrentRow.Cells("fldRotator").Value
        spnHQStator.Text = rgvHourly.CurrentRow.Cells("fldStator").Value
        spnHQSubstator.Text = rgvHourly.CurrentRow.Cells("fldSubStator").Value
        spnHQSleeve.Text = rgvHourly.CurrentRow.Cells("fldSleeve").Value
...
#Region "ASSY Line Eff"
    ...
    Private Sub AssyLineEffGrid()
        ...
        rgvAssyLineEff.Columns.Add(gridDeleteBtn)

        Dim fldTimePartition As New GridViewTextBoxColumn
        fldTimePartition.HeaderText = "Time Partition"
        ...
        rgvAssyLineEff.Columns.Add(fldTimePartition)
    ...
    Private Sub GetDefaultALETime()
        ...
        If Mio.Shift = "6AM-2PM(1st Shift)" Then
            ...
            'Dim TimeLine As String = Mio.AssyTimeFrom & "-" & Mio.AssyTimeTo
            'Dim fromTime As DateTime = DateTime.Parse(Mio.AssyTimeFrom)
            'Dim toTime As DateTime = DateTime.Parse(Mio.AssyTimeTo)
            'Dim TimeLineMin As Integer = CInt((toTime - fromTime).TotalMinutes)
    ...
    Private Sub btnALEAdd_Click(sender As Object, e As EventArgs) Handles btnALEAdd.Click
        If assyLineStatus = "EDIT" Then
            Dim result As DialogResult = RadMessageBox.Show("Are you sure you want to edit this record?", "QUESTION", MessageBoxButtons.YesNo, RadMessageIcon.Question)
            If result = DialogResult.Yes And selectedRowIndex >= 0 Then
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(1).Value = tbALETime.Text
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(2).Value = tbALECustomer.Text
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(3).Value = tbALEFGNo.Text
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(4).Value = cbModelName.Text
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(5).Value = tbALEModelCode.Text
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(6).Value = tbALELotNo.Text
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(7).Value = spnALEGoodQty.Value
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(8).Value = If(cbDelivered.Checked, "Delivered", "Not Delivered")
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(9).Value = spnALEReplace.Value
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(10).Value = spnALEScrap.Value
                rgvAssyLineEff.Rows(selectedRowIndex).Cells(11).Value = spnALESample.Value
            End If
            Dim result2 As DialogResult = RadMessageBox.Show("Edited Successfully!", "SUCCESS", MessageBoxButtons.OK, RadMessageIcon.Info)
        ...
        Else
            rgvAssyLineEff.Rows.Add("", tbALETime.Text, tbALECustomer.Text, tbALEFGNo.Text, cbModelName.Text, tbALEModelCode.Text, tbALELotNo.Text,
                                spnALEGoodQty.Value, If(cbDelivered.Checked, "Delivered", "Not Delivered"),
                                spnALEReplace.Value, spnALEScrap.Value, spnALESample.Value)
        End If
    ...
    Private Sub btnALESubmit_Click(sender As Object, e As EventArgs) Handles btnALESubmit.Click
        ...
        Else
            ...
            If Tohka.DeleteSrcTableByDRID("tblDailyReportAssyLineEff", Mio.DRID) = "Good" Then
                For rowcounter As Integer = 0 To Me.rgvAssyLineEff.Rows.Count - 1
                    Dim parOpTime As String = rgvAssyLineEff.Rows(rowcounter).Cells("fldTimePartition").Value.ToString()
                    Dim times() As String = parOpTime.Split("-"c)
                    Dim TotalOpTime As Integer = lblTotalOperationTime.Text
                    Dim startTime As TimeSpan = TimeSpan.Parse(times(0))
                    Dim endTime As TimeSpan = TimeSpan.Parse(times(1))

                    Dim diffMinutes As Integer = CInt((endTime - startTime).TotalMinutes)
                    If diffMinutes < 0 Then
                        diffMinutes += 1440
                    End If
                    Dim parTime As String = DirectCast(rgvAssyLineEff.Rows(rowcounter).Cells("fldTimePartition").Value, String)
                    ...
                    If Tohka.InsertSRCAssyLineEff(Mio.DRID, parTime, diffMinutes, spnALEHotto.Value, Convert.ToInt32(tbPlanned.Text), Convert.ToInt32(tbUnplanned.Text), Convert.ToDouble(tbCycle.Text), parCustomer, parFGNo, parModel, parModelCode, parLotNo, parGoodQty, parDelivered, parReplacement, parAssyScrap, parSample, TotalOpTime) = "Good" Then
    ...
    Private Sub rgvAssyLineEff_DoubleClick(sender As Object, e As EventArgs) Handles rgvAssyLineEff.DoubleClick
        ...
        selectedRowIndex = rgvAssyLineEff.CurrentRow.Index

        tbALETime.Text = rgvAssyLineEff.Rows(selectedRowIndex).Cells(1).Value.ToString()
        tbALECustomer.Text = rgvAssyLineEff.Rows(selectedRowIndex).Cells(2).Value.ToString()
        tbALEFGNo.Text = rgvAssyLineEff.Rows(selectedRowIndex).Cells(3).Value.ToString()
        cbModelName.Text = rgvAssyLineEff.Rows(selectedRowIndex).Cells(4).Value.ToString()
        tbALEModelCode.Text = rgvAssyLineEff.Rows(selectedRowIndex).Cells(5).Value.ToString()
        tbALELotNo.Text = rgvAssyLineEff.Rows(selectedRowIndex).Cells(6).Value.ToString()
        spnALEGoodQty.Value = (rgvAssyLineEff.Rows(selectedRowIndex).Cells(7).Value)
        cbDelivered.Checked = (rgvAssyLineEff.Rows(selectedRowIndex).Cells(8).Value.ToString() = "Delivered")
        spnALEReplace.Value = (rgvAssyLineEff.Rows(selectedRowIndex).Cells(9).Value)
        spnALEScrap.Value = (rgvAssyLineEff.Rows(selectedRowIndex).Cells(10).Value)
        spnALESample.Value = (rgvAssyLineEff.Rows(selectedRowIndex).Cells(11).Value)
...
#Region "LotNo"
    ...
    Private Sub GetLotNoTime()
        ...
        ddLotTime.Items.Add("Engg Sample")

        ddLotTime.SelectedIndex = 1
        ddLotTime.SelectedIndex = ddLotTime.SelectedIndex - 1
    ...
    Private Sub LotNoGrid()
        ...
        fldBoxNo.IsVisible = False
        rgvLotNo.Columns.Add(fldBoxNo)

        rgvLotNo.AutoSizeColumnsMode = GridViewAutoSizeColumnsMode.Fill


    End Sub
    Private Sub LotNoDataSource()
        Dim DTLotNoTemp As DataTable = Furina.MyQuery("SELECT fldPartName FROM tblDailyReportSRCRawMatsTemplate where fldModel='" & Mio.Model & "' and fldLine= '" & Mio.Line & "'")
        Dim DTLotNo As DataTable = Furina.MyQuery("SELECT fldPartName, fldBoxNo, fldLotNo, fldCavNo FROM tblDailyReportSRCRMWIP where fldDRID = '" & Mio.DRID & "'  and fldTime= '" & ddLotTime.Text & "' ORDER BY fldDRID LIMIT 1000")
        If ddLotTime.SelectedIndex <> -1 Then
            ...
            If dialogResult = DialogResult.Yes Then
                If DTLotNo.Rows.Count >= 1 Then
                    ...
                    txtBoxNo.Text = DTLotNo.Rows(0).Item("fldBoxNo")
                Else
            ...
            Else
                RemoveHandler ddLotTime.SelectedIndexChanged, AddressOf ddLotTime_SelectedIndexChanged
                ddLotTime.SelectedIndex = previousIndex
                AddHandler ddLotTime.SelectedIndexChanged, AddressOf ddLotTime_SelectedIndexChanged
            End If
        End If
    ...
    Private Sub ReloadLot()
        Dim DTLotNo As DataTable = Furina.MyQuery("SELECT fldPartName, fldCavNo, fldLotNo, fldBoxNo FROM tblDailyReportSRCRMWIP where fldDRID = '" & Mio.DRID & "'  and fldTime= '" & ddLotTime.Text & "' ORDER BY fldDRID LIMIT 1000")
        rgvLotNo.DataSource = DTLotNo
        txtBoxNo.Text = DTLotNo.Rows(0).Item("fldBoxNo")
    End Sub
    ...
    Private Sub InsertLot()
        ...
        '    'Dim cavNo As String = If(rgvLotNo.Rows(row).Cells("fldCavNo").Value, "")
        '    Dim lotNo As String = If(IsDBNull(rgvLotNo.Rows(row).Cells("fldLotNo").Value), "", rgvLotNo.Rows(row).Cells("fldLotNo").Value.ToString())
        '    Dim cavNo As String = If(IsDBNull(rgvLotNo.Rows(row).Cells("fldCavNo").Value), "", rgvLotNo.Rows(row).Cells("fldCavNo").Value.ToString())
        '    Dim partName As String = If(IsDBNull(rgvLotNo.Rows(row).Cells("fldPartName").Value), "", rgvLotNo.Rows(row).Cells("fldPartName").Value.ToString())
        ...
        '        Furina.AddMySqlParameters("parAddedBy", globalVariables.SRCAddedBy)
        ...
        Dim rotatorStr As String = String.Join(",", rotatorParts)
        Dim substatorStr As String = String.Join(",", substatorParts)
        Dim statorStr As String = String.Join(",", statorParts)
        Dim sleeveStr As String = String.Join(",", sleeveParts)
        ...
        '    MessageBox.Show(Tohka.InsertSRCHourly(Mio.DRID, ddLotTime.Text, 0, 0, 0, rotatorStr, statorStr, substatorStr, sleeveStr, "", 0, 0, 0, 0, 0, 0))
        '    ReloadHourlyGrid()
        'End If
    ...
    Private Sub btnLotSubmit_Click(sender As Object, e As EventArgs) Handles btnLotSubmit.Click
        Dim dtCheck = Furina.MyQuery("SELECT fldDRID, fldTime FROM dbMIA.tblDailyReportSRCRMWIP WHERE fldDRID = '" & Mio.DRID & "' AND fldTime = '" & ddLotTime.Text & "'")
        Dim deleteExist As String = Tohka.DeleteSrcLotByDRID(Mio.DRID, ddLotTime.Text)
        Try
    ...
    Private Sub ddLotTime_SelectedIndexChanged(sender As Object, e As Data.PositionChangedEventArgs)
        'RealoadLotNoGrid()
        LotNoDataSource()
        previousIndex = ddLotTime.SelectedIndex
    End Sub

    Private Sub rgvLotNo_KeyDown(sender As Object, e As KeyEventArgs)
        If e.KeyCode = Keys.Delete Then
            Dim result As Integer = MessageBox.Show("Are you sure you want to delete?", "Yes or No?", MessageBoxButtons.YesNo)
            If result = DialogResult.No Then
            Else
    ...
    Private Sub rgvLotNo_CellDoubleClick(sender As Object, e As GridViewCellEventArgs)
    ...
    Private Sub SubtractActualPlan()
        spnHQDeviation.Value = spnHQPlan.Value - spnHQActual.Value
    End Sub

    Private Sub rgvAssyLineEff_CellClick(sender As Object, e As GridViewCellEventArgs) Handles rgvAssyLineEff.CellClick
        globalVariables.currentRgv = rgvAssyLineEff
        ...
        If e.Column IsNot Nothing AndAlso e.Column.Name = "DELETE" AndAlso e.RowIndex >= 0 Then
            assyLineStatus = "DELETE"
            Dim selectedRow As GridViewRowInfo = rgvAssyLineEff.CurrentRow
        ...
        RadMessageIcon.Question)
            ...
            If result = DialogResult.Yes Then
                rgvAssyLineEff.Rows.RemoveAt(selectedRowIndex)
    ...
    Private Sub rcbQualified_CheckStateChanged(sender As Object, e As EventArgs) Handles rcbQualified.CheckStateChanged
        If rcbQualified.CheckState = CheckState.Checked Then
            parQualify = 1
        ElseIf rcbQualified.CheckState = CheckState.Unchecked Then
            parQualify = 0
        End If
    End Sub

    Private Sub rcbTransfer_CheckStateChanged(sender As Object, e As EventArgs) Handles rcbTransfer.CheckStateChanged
        If rcbTransfer.CheckState = CheckState.Checked Then
            parTransfer = 1
        ElseIf rcbTransfer.CheckState = CheckState.Unchecked Then
            parTransfer = 0
        End If
    End Sub

    Private Sub rcbTrainee_CheckStateChanged(sender As Object, e As EventArgs) Handles rcbTrainee.CheckStateChanged
        If rcbTrainee.CheckState = CheckState.Checked Then
            parTrainee = 1
        ElseIf rcbTrainee.CheckState = CheckState.Unchecked Then
            parTrainee = 0
        End If
    End Sub

    Private Sub RadPanel12_Paint(sender As Object, e As PaintEventArgs)
