...
Public Class frmProcessList
    ...
    Private Sub FrmProcess_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If (e.KeyCode = Keys.Enter AndAlso e.Control) Then
            cbbAddRecord.PerformClick()
        End If
    End Sub
    ...
    Private Sub readyGrid()
        ...
        fldProcessName.Width = 350
        gvData.Columns.Add(fldProcessName)

        Dim fldPriority As New GridViewDecimalColumn("PRIORITY")
        fldPriority.FieldName = "fldPriority"
        fldPriority.Width = 90
        fldPriority.TextAlignment = ContentAlignment.MiddleCenter
        gvData.Columns.Add(fldPriority)

        Dim fldColor As New GridViewColorColumn("COLOR")
        fldColor.FieldName = "fldColor"
        fldColor.Width = 150
        gvData.Columns.Add(fldColor)

        Dim fldHideInBOM As New GridViewCheckBoxColumn("HIDE IN BOM")
        fldHideInBOM.FieldName = "fldHideInBOM"
        fldHideInBOM.Width = 100
        gvData.Columns.Add(fldHideInBOM)
    ...
    Private Sub bwLoadData_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bwLoadData.DoWork
        myData = Slipknot.dbSelect("SP_SelectAllProcess")
    End Sub
    ...
    Private Sub FrmProcess_Shown(sender As Object, e As EventArgs) Handles Me.Shown
    ...
    Private Sub cbbAddRecord_Click(sender As Object, e As EventArgs) Handles cbbAddRecord.Click
        Dim check As String = Slipknot.checkPermission(currentUser.Item("fldUsername"), "fldMRProcessList")

        If check = "RW" Then
            If txtCode.Text.TrimStart.TrimEnd = String.Empty Then
                RadMessageBox.Show("Code should not be empty", "WARNING", MessageBoxButtons.OK, RadMessageIcon.Exclamation)

                Exit Sub
            End If

            If txtProcessName.Text.TrimStart.TrimEnd = String.Empty Then
                RadMessageBox.Show("Process name should not be empty", "WARNING", MessageBoxButtons.OK, RadMessageIcon.Exclamation)
            ...
            If Slipknot.dbAED("SP_InsertProcess") Then
    ...
    Private Sub bwOperation_DoWork(sender As Object, e As DoWorkEventArgs) Handles bwOperation.DoWork
        ...
        Slipknot.dbAED("SP_DeleteProcess")
    End Sub
    ...
    Private Sub cbbEdit_Click(sender As Object, e As EventArgs) Handles cbbEdit.Click
        ...
        If check = "RW" Then
            ...
            If gvData.SelectedRows.Count > 0 Then
                If Slipknot.IsFormOpen(FrmEditProcess) Then
                    FrmEditProcess.Focus()
                Else
                    If Convert.ToString(gvData.CurrentRow.Cells("fldId").Value) <> String.Empty Then
                        FrmEditProcess.txtID.Text = Convert.ToString(gvData.CurrentRow.Cells("fldId").Value)
                        FrmEditProcess.Show(Me)
                    End If
