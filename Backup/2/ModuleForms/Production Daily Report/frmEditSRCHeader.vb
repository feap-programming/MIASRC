    ...
    Private Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        Try
            ...
            If tpFrom.Value > tpTo.Value Or ddShift.SelectedIndex = 3 Then
                DTTo = Convert.ToDateTime(Year(dpDate.Value) & "-" & Month(dpDate.Value) & "-" & DateAdd("d", 1, dpDate.Value).ToString("dd") & " " & DateToStr)
            Else
                DTTo = Convert.ToDateTime(Year(dpDate.Value) & "-" & Month(dpDate.Value) & "-" & dpDate.Value.ToString("dd") & " " & DateToStr)
            End If
