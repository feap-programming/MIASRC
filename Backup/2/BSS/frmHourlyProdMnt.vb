...
Public Class FrmHourlyProdMnt
    Private Sub FrmHourlyProdMnt_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        loadShift()
        readyGrid()
        mcbShift.Text = globalVariables.ShiftSelect
    End Sub
    ...
    Public Sub FetchHourlyProdMnt()
        ...
        Dim dtHourlyProdData As DataTable = Slipknot.dbSelect("SP_SelectHourlyProdMntbyPOSPID")

        If dtHourlyProdData.Rows.Count = 0 Then
            Slipknot.addParameter("MainId", txtMainID.Text)
            Slipknot.addParameter("POSPID", txtPOSPID.Text)
            Slipknot.addParameter("ShiftName", mcbShift.Text)
            Slipknot.dbAED("SP_InsertBSSShiftPerHour")
    ...
    Private Sub cbbDelete_Click(sender As Object, e As EventArgs) Handles cbbDelete.Click
        Dim result As DialogResult = RadMessageBox.Show(Me, "this is irreversible, Are you sure you want to delete this record?", "Deleting Records", MessageBoxButtons.YesNo, RadMessageIcon.Question, MessageBoxDefaultButton.Button2)
        If result = DialogResult.Yes Then
            Slipknot.addParameter("HID", gvData.CurrentRow.Cells("fldHID").Value.ToString)
            Slipknot.dbAED("SP_DeleteHourdlyProdbyHID")

            RadMessageBox.Show("Deleted Successfully!", "INFORMATION", MessageBoxButtons.OK, RadMessageIcon.Info)
            FetchData()
        End If
    End Sub
