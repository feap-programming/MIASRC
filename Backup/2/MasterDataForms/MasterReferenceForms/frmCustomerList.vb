...
Public Class FrmCustomerList
    ...
    Private Sub readyGrid()
        ...
        Dim fldCustomerCode As New GridViewTextBoxColumn("CUSTOMER NAME")
        fldCustomerCode.FieldName = "fldCustomerName"
        fldCustomerCode.Width = 140
        gvData.Columns.Add(fldCustomerCode)

        Dim fldCustomerName As New GridViewTextBoxColumn("CUSTOMER CODE")
        fldCustomerName.FieldName = "fldCustomerCode"
        fldCustomerName.Width = 160
    ...
    Private Sub bwLoadData_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bwLoadData.DoWork
        myData = Slipknot.dbSelect("SP_SelectAllCustomers")
    End Sub
    ...
    Private Sub FrmCustomerList_Shown(sender As Object, e As EventArgs) Handles Me.Shown
    ...
    Private Sub bwOperation_DoWork(sender As Object, e As DoWorkEventArgs) Handles bwOperation.DoWork
        ...
        Slipknot.dbAED("SP_DeleteCustomer")
    End Sub
    ...
    Private Sub cbbAddRecord_Click(sender As Object, e As EventArgs) Handles cbbAddRecord.Click
        ...
        Dim check As String = Slipknot.checkPermission(currentUser.Item("fldUsername"), "fldMRCustomerList")

        If check = "RW" Then

            If txtCustomerCode.Text.TrimStart.TrimEnd = String.Empty Then
                RadMessageBox.Show("Customer code. should not be empty", "WARNING", MessageBoxButtons.OK, RadMessageIcon.Exclamation)
                Exit Sub
            End If

            If txtCustomerName.Text.TrimStart.TrimEnd = String.Empty Then
                RadMessageBox.Show("Customer name should not be empty", "WARNING", MessageBoxButtons.OK, RadMessageIcon.Exclamation)
                Exit Sub
            End If

            Slipknot.addParameter("parCustomerCode", txtCustomerCode.Text.TrimStart.TrimEnd)
            Slipknot.addParameter("parCustomerName", txtCustomerName.Text.TrimStart.TrimEnd)
            Slipknot.addParameter("parAddedBy", currentUser.Item("fldUsername"))

            If Slipknot.dbAED("SP_InsertCustomer") Then
