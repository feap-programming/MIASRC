...
Public Class FrmWeldingUpload
    ...
    Dim dtSampleFwTemp As New DataTable
    ...
    Private Sub FrmWeldingUpload_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If Mio.uploadType = "FW" Then
            gbSampleFW.Visible = True
        Else
            gbSampleFW.Visible = False
        End If
    End Sub
    Private Sub CleanUp()
        ...
        gvOtherDetails.DataSource = Nothing
        gvFwSample.DataSource = Nothing

        dtManpower.Reset()
        dtGoodQty.Reset()
        dtGoodQtyTemp.Reset()
        dtSampleFwTemp.Reset()
        dtDefects.Reset()
        dtLotNo.Reset()
        dtLotNoTemp.Reset()
        dtHourly.Reset()
        dtHourlyTemp.Reset()
        dtOtherDetails.Reset()
        dtSampleFw.Reset()
        ExcelManpower.Reset()
        ExcelGoodQty.Reset()
        ExcelHourly.Reset()
        ExcelDefect.Reset()
        ExcelOtherDetails.Reset()
        ExcelLotNo.Reset()
        ExcelFwSample.Reset()
    End Sub
    ...
    Private Sub cbbOpen_Click(sender As Object, e As EventArgs) Handles cbbOpen.Click
        ...
        Else
            ...
            dtSampleFwTemp.Columns.Add("Part Name")
            dtSampleFwTemp.Columns.Add("Quantity")

            'dtSampleFw.Columns.Add("Part Name")
            'dtSampleFw.Columns.Add("Quantity")
            ...
            Using con As OleDb.OleDbConnection = New OleDb.OleDbConnection()
                ...
                'con.ConnectionString = String.Format("Provider=Microsoft.ACE.OLEDB.12.0;Data Source={0};Extended Properties='Excel 12.0 Xml;HDR=YES;IMEX=1';", path)
                ...
                '--------------FW SAMPLE---------------
                If Mio.uploadType = "FW" Then
                    Dim OwOString2 As String = "SELECT * FROM [FW SAMPLE$]"
                    Using cmd As OleDb.OleDbCommand = New OleDb.OleDbCommand(OwOString2, con)
                        Using da As OleDb.OleDbDataAdapter = New OleDb.OleDbDataAdapter(cmd)
                            con.Open()
                            da.Fill(dtSampleFw)
                            con.Close()
                        End Using
                    End Using
                    Dim Adrestia2 As Integer = 1

                    If dtSampleFw.Rows.Count > 0 Then
                        For Each col As DataColumn In dtSampleFw.Columns
                            Dim partName As String = col.ColumnName
                            Dim quantity As String = dtSampleFw.Rows(0)(col).ToString()
                            dtSampleFwTemp.Rows.Add(partName, quantity)
                        Next
                    End If
                ...
                '--------------- Other Details ---------------
                Using cmd As OleDb.OleDbCommand = New OleDb.OleDbCommand("SELECT * FROM [OTHER DETAILS$]", con)
                    Using da As OleDb.OleDbDataAdapter = New OleDb.OleDbDataAdapter(cmd)
                        con.Open()
    ...
    Private Sub BackgroundWorker1_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker1.DoWork
        ...
        Else
            Dim DTWip As DataTable = Maria.MyQuery("SELECT * FROM tblDailyReportWeldingTemplate where fldModel='" & globalVariables.DailyReportModel & "' and fldType='" & globalVariables.SRCReportType & "' and fldLine ='" & globalVariables.ModelLineWelding & "'")
        ...
        ExcelOtherDetails = dtOtherDetails

        Threading.Thread.Sleep(80)
        CurrentProg = "Opening Other Details"
        OpenProgress = OpenProgress + 14
        percent = OpenProgress
        BackgroundWorker1.ReportProgress(Convert.ToInt32(percent))
        ...
        If Mio.uploadType = "FW" Then
            ...
            fldPartName.FieldName = "Part Name"
            fldPartName.TextAlignment = ContentAlignment.MiddleCenter
            fldPartName.Width = 150
            gvFwSample.Columns.Add(fldPartName)

            Dim Quantity As New GridViewTextBoxColumn
            Quantity.HeaderText = "Quantity"
            Quantity.FieldName = "Quantity"
            Quantity.TextAlignment = ContentAlignment.MiddleCenter
            Quantity.Width = 150
            gvFwSample.Columns.Add(Quantity)

            ExcelFwSample = dtSampleFwTemp

            Threading.Thread.Sleep(80)
            CurrentProg = "Opening FW Sample"
            OpenProgress = OpenProgress + 14
            percent = OpenProgress
            BackgroundWorker1.ReportProgress(Convert.ToInt32(percent))
    ...
    Private Sub BackgroundWorker1_RunWorkerCompleted(sender As Object, e As System.ComponentModel.RunWorkerCompletedEventArgs) Handles BackgroundWorker1.RunWorkerCompleted
        ...
        gvOtherDetails.DataSource = ExcelOtherDetails
        gvFwSample.DataSource = ExcelFwSample
    ...
    Private Sub BackgroundWorker2_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker2.DoWork
        ...
        While Manpowerint <= gvDataManpower.Rows.Count - 1
            ...
            Threading.Thread.Sleep(80)
            Dim Bruh As Decimal = 14 / gvDataManpower.Rows.Count
        ...
        While GoodQtyint <= gvGQ.Rows.Count - 1
            ...
            Threading.Thread.Sleep(80)
            Dim Bruh As Decimal = 14 / gvGQ.Rows.Count
        ...
        For rowcounter As Integer = 0 To Me.gvHourly.Rows.Count - 1
            Dim deviation As Integer = Convert.ToInt32(gvHourly.Rows(rowcounter).Cells(3).Value) - Convert.ToInt32(gvHourly.Rows(rowcounter).Cells(2).Value)
            ...
            Maria.AddMySqlParameters("parDeviation", Convert.ToInt32(gvHourly.Rows(rowcounter).Cells(3).Value) - Convert.ToInt32(gvHourly.Rows(rowcounter).Cells(2).Value))
            ...
            Threading.Thread.Sleep(80)
            Dim Bruh As Decimal = 14 / gvHourly.Rows.Count
        ...
        If globalVariables.SRCReportType = "FW" Then
            ...
            For rowcounter As Integer = 0 To Me.gvDefect.Rows.Count - 1
                ...
                Threading.Thread.Sleep(80)
                Dim Bruh As Decimal = 14 / gvDefect.Rows.Count
        ...
        Else
            ...
            For rowcounter As Integer = 1 To Me.gvDefect.Rows.Count

                For colcounter As Integer = 1 To Me.gvDefect.Columns.Count - 1
                    Try
                        Maria.AddMySqlParameters("parDRID", globalVariables.DailyRepID)
                        Maria.AddMySqlParameters("parType", globalVariables.SRCReportType)
                        Maria.AddMySqlParameters("parDefects", gvDefect.Rows(rowcounter - 1).Cells(0).Value.ToString)
                        Maria.AddMySqlParameters("parWIP", gvDefect.Columns(colcounter).FieldName)
                        Maria.AddMySqlParameters("parQty", gvDefect.Rows(rowcounter - 1).Cells(colcounter).Value)
                        Maria.AddMySqlParameters("parTableNo", 1)
                        Maria.AddMySqlParameters("parAddedBy", globalVariables.currentUser(1))
        ...
        Maria.MyQuery("SET SQL_SAFE_UPDATES = 0;Delete from dbMIA.tblDailyReportWeldingOtherDetails where fldDRID=" & globalVariables.DailyRepID & ";SET SQL_SAFE_UPDATES = 1;")

        For rowcounter As Integer = 0 To Me.gvOtherDetails.Rows.Count - 1

            For colcounter As Integer = 0 To Me.gvOtherDetails.Columns.Count - 1
                Try
                    Maria.AddMySqlParameters("parDRID", globalVariables.DailyRepID)
                    Maria.AddMySqlParameters("parOtherDetails", gvOtherDetails.Rows(rowcounter).Cells(0).Value)

                    Maria.SPADE("SP_SRCDailyReport_InsertWeldingOtherDetails")
                Catch ex As Exception
                    MessageBox.Show(ex.Message)
                End Try
            Next
            Threading.Thread.Sleep(80)
            Dim Bruh As Decimal
            Bruh = 14 / gvOtherDetails.Rows.Count
            CurrentProg = "Saving Other Details"
            OpenProgress = OpenProgress + Bruh
            percent = OpenProgress
            BackgroundWorker2.ReportProgress(Convert.ToInt32(percent))
        Next
        If globalVariables.SRCReportType = "FW" Then
            Maria.MyQuery("SET SQL_SAFE_UPDATES = 0;Delete from dbMIA.tblDailyReportWeldingSample where fldDRID=" & globalVariables.DailyRepID & ";SET SQL_SAFE_UPDATES = 1;")
            For rowcounter As Integer = 0 To Me.gvFwSample.Rows.Count - 1
                Try
                    Maria.AddMySqlParameters("parDRID", globalVariables.DailyRepID)
                    Maria.AddMySqlParameters("parModel", globalVariables.DailyReportModel)
                    Maria.AddMySqlParameters("parPartName", gvFwSample.Rows(rowcounter).Cells(0).Value)
                    Maria.AddMySqlParameters("parQuantity", gvFwSample.Rows(rowcounter).Cells(1).Value)
                    Maria.AddMySqlParameters("parLine", globalVariables.ModelLineWelding)
                    Maria.AddMySqlParameters("parDateAdded", DateTime.Now())
                    Maria.AddMySqlParameters("parAddedBy", globalVariables.currentUser(1))

                    Maria.SPADE("SP_SRCDailyReport_InsertWeldingSampleFw")
                Catch ex As Exception
                    MessageBox.Show(ex.Message)
                End Try
                Threading.Thread.Sleep(80)
                Dim Bruh As Decimal = 14 / gvFwSample.Rows.Count
                CurrentProg = "Saving FW Sample"
                OpenProgress = OpenProgress + Bruh
                percent = OpenProgress
                BackgroundWorker2.ReportProgress(Convert.ToInt32(percent))
            Next
        ...
        For rowcounter As Integer = 0 To Me.gvLot.Rows.Count - 1
            ...
            Threading.Thread.Sleep(80)
            Dim Bruh As Decimal = 14 / gvLot.Rows.Count
