...
Public Class FrmMakerList
    Private myData As DataTable
    Private Sub FrmMakerList_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub readyGrid()
        ...
        fldMakerCode.Width = 120
        gvData.Columns.Add(fldMakerCode)

        Dim fldMakerName As New GridViewTextBoxColumn("MAKER NAME")
        fldMakerName.FieldName = "fldMakerName"
        fldMakerName.Width = 160
        gvData.Columns.Add(fldMakerName)
    ...
    Private Sub bwLoadData_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bwLoadData.DoWork
        ...
        myData = Slipknot.dbSelect("SP_SelectAllMaker")
    ...
    Private Sub FrmMakerList_Shown(sender As Object, e As EventArgs) Handles Me.Shown
    ...
    Private Sub cbbAdd_Click(sender As Object, e As EventArgs) Handles cbbAdd.Click
        ...
        Dim check As String = Slipknot.checkPermission(currentUser.Item("fldUsername"), "fldMRMakerList")

        If check = "RW" Then
            If txtCode.Text.TrimStart.TrimEnd = String.Empty Then
                RadMessageBox.Show("Maker code cannot not be empty", "WARNING", MessageBoxButtons.OK, RadMessageIcon.Exclamation)
                Exit Sub
            End If

            If txtMakerName.Text.TrimStart.TrimEnd = String.Empty Then
                RadMessageBox.Show("Maker name cannot not be empty", "WARNING", MessageBoxButtons.OK, RadMessageIcon.Exclamation)
                Exit Sub
            End If

            Slipknot.addParameter("parCode", txtCode.Text.TrimStart.TrimEnd)
            Slipknot.addParameter("parMakerName", txtMakerName.Text.TrimStart.TrimEnd)
            Slipknot.addParameter("parAddedBy", currentUser.Item("fldUsername"))
            If Slipknot.dbAED("SP_InsertMaker") Then
