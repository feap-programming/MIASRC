...
Public Class FrmModelList
    ...
    Private Sub FrmModelList_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        bwLoadData.RunWorkerAsync()
    End Sub

    Private Sub bwLoadData_DoWork(sender As Object, e As DoWorkEventArgs) Handles bwLoadData.DoWork
        myData = Slipknot.dbSelect("SP_SelectAllModel")
    End Sub
    ...
    Private Sub cbbAdd_Click(sender As Object, e As EventArgs) Handles cbbAdd.Click
        ...
        Dim check As String = Slipknot.checkPermission(currentUser.Item("fldUsername"), "fldMRModelList")
        ...
        If check = "RW" Then
            ...
            If txtModelName.Text.TrimStart.TrimEnd = String.Empty Then
                RadMessageBox.Show("Model name cannot be empty", "WARNING", MessageBoxButtons.OK, RadMessageIcon.Exclamation)

                Exit Sub
            End If

            Slipknot.addParameter("parCode", txtCode.Text.TrimStart.TrimEnd)
            Slipknot.addParameter("parModelName", txtModelName.Text.TrimStart.TrimEnd)
            Slipknot.addParameter("parAddedBy", currentUser.Item("fldUsername"))
            If Slipknot.dbAED("SP_InsertModel") Then
