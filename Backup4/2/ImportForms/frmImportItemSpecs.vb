...
Public Class frmImportItemSpecs
    Private Sub frmImportItemSpecs_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub readyGrid()
        ...
        gvData.Columns("fldCav").FormatString = "{0:N2}"

        Dim fldPartWeight As New GridViewDecimalColumn("PART WEIGHT")
        fldPartWeight.FieldName = "fldPartWeight"
        fldPartWeight.Width = 110
        fldPartWeight.ThousandsSeparator = True
        gvData.Columns.Add(fldPartWeight)
        gvData.Columns("fldPartWeight").FormatString = "{0:N2}"
        ...
        fldRunnerWeight.ThousandsSeparator = True
        gvData.Columns.Add(fldRunnerWeight)
        gvData.Columns("fldRunnerWeight").FormatString = "{0:N2}"

        Dim fldLoss As New GridViewDecimalColumn("LOSS")
        fldLoss.FieldName = "fldLoss"
        fldLoss.Width = 70
        fldLoss.ThousandsSeparator = True
        gvData.Columns.Add(fldLoss)
        gvData.Columns("fldLoss").FormatString = "{0:N2}"

        Dim fldCollection As New GridViewDecimalColumn("COLLECTION")
        fldCollection.FieldName = "fldCollection"
        fldCollection.Width = 100
        fldCollection.ThousandsSeparator = True
        gvData.Columns.Add(fldCollection)
        gvData.Columns("fldCollection").FormatString = "{0:N2}"

        Dim fldCycleTime As New GridViewDecimalColumn("CYCLE TIME")
        fldCycleTime.FieldName = "fldCycleTime"
        fldCycleTime.Width = 100
        fldCycleTime.ThousandsSeparator = True
        ...
        Try
            ...
            Dim joinResult As IEnumerable = (From T0 In table.AsEnumerable()
                                             Select New With
                                             {
                                               ...
                                               .fldCav = T0.Item(1),
                                               .fldPartWeight = T0.Item(2),
                                               .fldRunnerWeight = T0.Item(3),
                                               .fldLoss = T0.Item(4),
                                               .fldCollection = T0.Item(5),
                                               .fldCycleTime = T0.Item(6),
                                               .fldEffectivityDate = T0.Item(7),
                                               .fldItemCodeOk = checkItemCode(.fldItemCode),
                                               .STATUS = checkNG(.fldItemCodeOk)
                                             }).ToList
    ...
    Function checkNG(ByVal itemCodeOk1 As Boolean) As String

        If itemCodeOk1 = False Then
            Return "NG"
    ...
    Private Sub gvData_CellFormatting(sender As Object, e As CellFormattingEventArgs) Handles gvData.CellFormatting
        ...
        Else
            ...
            If Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldItemCodeOk").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Item code not found in item master data"
            Else
    ...
    Private Sub bwSave_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bwSave.DoWork
        ...
        For counter As Integer = 0 To gvData.Rows.Count
            ...
            If Convert.ToString(gvData.Rows(counter).Cells("STATUS").Value) <> "NG" Then
                ...
                cav = Convert.ToDecimal(gvData.Rows(counter).Cells("fldCav").Value)
                partWeight = Convert.ToDecimal(gvData.Rows(counter).Cells("fldPartWeight").Value)
                runnerWeight = Convert.ToDecimal(gvData.Rows(counter).Cells("fldRunnerWeight").Value)
                loss = Convert.ToDecimal(gvData.Rows(counter).Cells("fldLoss").Value)
                collection = Convert.ToDecimal(gvData.Rows(counter).Cells("fldCollection").Value)
                cycleTime = Convert.ToDecimal(gvData.Rows(counter).Cells("fldCycleTime").Value)
                ...
                Slipknot.addParameter("parCav", cav)
