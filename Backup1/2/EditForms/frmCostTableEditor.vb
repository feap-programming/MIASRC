...
Public Class FrmCostTableEditor

    Private ucIM As New ucIMEditor
    Private ucNotIM As New ucCTEditor
    'Private gvData As New RadGridView
    Private Sub FrmCostTableEditor_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        Slipknot.changeTheme(Me)

        loadRecord()

        If txtCategory.Text = "IM" Then

            'scrPanel.Controls.Add(gvData)
            ...
            'AddHandler gvData.CellEndEdit, AddressOf gvData_CellEndEdit
            'AddHandler gvData.CellFormatting, AddressOf gvData_CellFormatting

        Else
            scrPanel.Controls.Add(ucNotIM)
            scrPanel.Dock = DockStyle.Fill
            scrPanel.BringToFront()
        End If
    ...
    Private Sub loadRecord()
        ...
        If dt.Rows.Count > 0 Then
            ...
            txtCategory.Text = Convert.ToString(dt.Rows(0).Item("fldCategory"))

        End If

    End Sub

    Private Sub btnFormulaSettings_Click(sender As Object, e As EventArgs) Handles btnFormulaSettings.Click

        If Slipknot.IsFormOpen(FrmCTFormulaSettings) Then
            FrmCTFormulaSettings.Focus()
        Else
            FrmCTFormulaSettings.Show(Me)
        End If

    End Sub

    Private Sub btnUnitPriceOfMTMH_Click(sender As Object, e As EventArgs) Handles btnUnitPriceOfMTMH.Click


        If Slipknot.IsFormOpen(frmUnitPriceForMTMH) Then
            frmUnitPriceForMTMH.Focus()
        Else
            frmUnitPriceForMTMH.Show(Me)
        End If

    End Sub

    Private Sub FrmCostTableEditor_Shown(sender As Object, e As EventArgs) Handles Me.Shown
    ...
    Private Sub saveIMCostTable()

        For Each row As GridViewRowInfo In gvData.Rows

            Slipknot.addParameter("parId", row.Cells("fldId").Value)
    ...
    Private Sub saveNotIMCostTable()

        For Each row As GridViewRowInfo In ucNotIM.gvSection1.Rows

            Slipknot.addParameter("parId", row.Cells("fldId").Value)
            Slipknot.addParameter("parCavity", row.Cells("fldCavity").Value)
            Slipknot.addParameter("parPartWeight", row.Cells("fldPartWeight").Value)
            Slipknot.addParameter("parRunnerWeight", row.Cells("fldRunnerWeight").Value)
            Slipknot.addParameter("parLoss", row.Cells("fldLoss").Value)
            Slipknot.addParameter("parUnitPrice", row.Cells("fldUnitPrice").Value)
            Slipknot.addParameter("parCollection", row.Cells("fldCollection").Value)
            Slipknot.addParameter("parScrapUnitPrice", row.Cells("fldScrapUnitPrice").Value)
            Slipknot.addParameter("parCycleTime", row.Cells("fldCycleTime").Value)
            Slipknot.addParameter("parMhHr", row.Cells("fldMhHr").Value)

            Slipknot.dbAED("SP_UpdateCTSection1")

        Next

        For Each row As GridViewRowInfo In ucNotIM.gvSection2.Rows

            Slipknot.addParameter("parId", row.Cells("fldId").Value)
            Slipknot.addParameter("parAtJapan", row.Cells("fldAtJapan").Value)
            Slipknot.addParameter("parLoss", row.Cells("fldLoss").Value)
            Slipknot.addParameter("parUnitPrice", row.Cells("fldUnitPrice").Value)

            Slipknot.dbAED("SP_UpdateCTSection2")

        Next

        For Each row As GridViewRowInfo In ucNotIM.gvSection3.Rows

            Slipknot.addParameter("parId", row.Cells("fldId").Value)
            Slipknot.addParameter("parCycle", row.Cells("fldCycle").Value)
            Slipknot.addParameter("parOperator", row.Cells("fldOperator").Value)
            Slipknot.addParameter("parOperation", row.Cells("fldOperation").Value)
            Slipknot.addParameter("parLoss", row.Cells("fldLoss").Value)

            Slipknot.dbAED("SP_UpdateCTSection3")

        Next

        For Each row As GridViewRowInfo In ucNotIM.gvSection4.Rows

            Slipknot.addParameter("parID", row.Cells("fldId").Value)
            Slipknot.addParameter("parValue", row.Cells("fldValue").Value)

            Slipknot.dbAED("SP_UpdateCTSection4")
    ...
    Private Sub FrmCostTableEditor_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub btnViewReport_Click(sender As Object, e As EventArgs) Handles btnViewReport.Click
        ...
        Slipknot.addReportParameter("parCITD", txtId.Text)
        Slipknot.addReportParameter("parYear", txtYear.Text)
        Slipknot.addReportParameter("parPartName", txtPartName.Text)
        Slipknot.addReportParameter("parCustomerPartNo", txtCustomerPartNo.Text)
        Slipknot.addReportParameter("parCustomer", txtCustomer.Text)
    ...
    '        dtFinal.Rows.Add(childCode, usage)
    ...
    Private Sub loadCostTableIM()
        ...
        Dim dtBOM As DataTable = Slipknot.dbSelectManual("SELECT " _
                                                       ...
                                                       & "FROM " _
                                                       & "tblBOMMaster T0 " _
                                                       & "WHERE T0.fldParentCode = '" & txtItemCode.Text & "' ")
        ...
        Dim myData As DataTable = Slipknot.dbSelectManual("SELECT " _
                                                        ...
                                                        & "WHERE FIND_IN_SET(T0.fldItemCode, '" & itemCodeArray & "') " _
                                                        & "")
