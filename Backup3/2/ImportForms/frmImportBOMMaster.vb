...
Public Class frmImportBOMMaster
    Private Sub frmImportBOMMaster_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub readyGrid()
        ...
        Dim fldParentItemCode As New GridViewTextBoxColumn("PARENT ITEM CODE")
        fldParentItemCode.FieldName = "fldParentItemCode"
        fldParentItemCode.Width = 150
        gvData.Columns.Add(fldParentItemCode)

        Dim fldChildItemCode As New GridViewTextBoxColumn("CHILD ITEM CODE")
        fldChildItemCode.FieldName = "fldChildItemCode"
        fldChildItemCode.Width = 150
        gvData.Columns.Add(fldChildItemCode)

        Dim fldUsage As New GridViewDecimalColumn("USAGE")
        fldUsage.FieldName = "fldUsage"
        fldUsage.Width = 140
        gvData.Columns.Add(fldUsage)
        gvData.Columns("fldUsage").FormatString = "{0:N5}"
        ...
        Dim fldItemCodeOk2 As New GridViewCheckBoxColumn("ITEM CODE OK 2")
        fldItemCodeOk2.FieldName = "fldItemCodeOk2"
        fldItemCodeOk2.Width = 100
        fldItemCodeOk2.IsVisible = False
        gvData.Columns.Add(fldItemCodeOk2)
    ...
    Private Sub cbbPaste_Click(sender As Object, e As EventArgs) Handles cbbPaste.Click
        ...
        Try
            ...
            Dim joinResult As IEnumerable = (From T0 In table.AsEnumerable()
                                             Select New With
                                             {
                                               .fldParentItemCode = T0.Item(0),
                                               .fldChildItemCode = T0.Item(1),
                                               .fldUsage = T0.Item(2),
                                               .fldRemarks = T0.Item(3),
                                               .fldItemCodeOk = checkItemCode(.fldParentItemCode),
                                               .fldItemCodeOk2 = checkItemCode(.fldChildItemCode),
                                               .STATUS = checkNG(.fldItemCodeOk, .fldItemCodeOk2)
                                             }).ToList
    ...
    Private Sub gvData_CellFormatting(sender As Object, e As CellFormattingEventArgs) Handles gvData.CellFormatting
        ...
        Else
            ...
            If Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldItemCodeOk").Value) = False And Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldItemCodeOk2").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Parent item code not found in master list, child item code not found in master list"
            ElseIf Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldItemCodeOk").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Parent item code not found in master list"
            ElseIf Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldItemCodeOk2").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Child item code not found in list"
            Else
    ...
    Private Sub bwSave_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bwSave.DoWork
        ...
        For counter As Integer = 0 To gvData.Rows.Count
            ...
            If Convert.ToString(gvData.Rows(counter).Cells("STATUS").Value) <> "NG" Then
                ...
                parentItemCode = Convert.ToString(gvData.Rows(counter).Cells("fldParentItemCode").Value)
                childItemCode = Convert.ToString(gvData.Rows(counter).Cells("fldChildItemCode").Value)
                usage = Convert.ToString(gvData.Rows(counter).Cells("fldUsage").Value)
                remarks = Convert.ToString(gvData.Rows(counter).Cells("fldRemarks").Value)

                Slipknot.addParameter("parParent", parentItemCode)
                Slipknot.addParameter("parChild", childItemCode)
