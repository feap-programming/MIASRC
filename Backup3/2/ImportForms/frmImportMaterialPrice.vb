...
Public Class frmImportMaterialPrice
    Private Sub frmImportItemPerModel_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub readyGrid()
        ...
        Dim fldPriceOk As New GridViewCheckBoxColumn("PRICE OK")
        fldPriceOk.FieldName = "fldPriceOk"
        fldPriceOk.Width = 100
        fldPriceOk.IsVisible = False
        gvData.Columns.Add(fldPriceOk)

        Dim fldClassificationOk As New GridViewCheckBoxColumn("CLASSIFICATION OK")
        fldClassificationOk.FieldName = "fldClassOk"
        fldClassificationOk.Width = 100
        fldClassificationOk.IsVisible = False
        gvData.Columns.Add(fldClassificationOk)

        Dim fldCurrencyOk As New GridViewCheckBoxColumn("CURRENCY OK")
        fldCurrencyOk.FieldName = "fldCurrencyOk"
        fldCurrencyOk.Width = 100
        fldCurrencyOk.IsVisible = False
        gvData.Columns.Add(fldCurrencyOk)
        ...
        Try
            ...
            Dim joinResult As IEnumerable = (From T0 In table.AsEnumerable()
                                             Select New With
                                             {
                                               ...
                                               .fldPrice = T0.Item(1),
                                               .fldClassification = T0.Item(2),
                                               .fldCurrency = T0.Item(3),
                                               .fldEffectivityDate = T0.Item(4),
                                               .fldItemCodeOk = checkItemCode(.fldItemCode),
                                               .fldPriceOk = checkPrice(.fldPrice),
                                               .fldClassOk = checkClassification(.fldClassification),
                                               .fldCurrencyOk = checkCurrency(.fldCurrency),
                                               .STATUS = checkNG(.fldItemCodeOk, .fldPriceOk, .fldClassOk, .fldCurrencyOk)
                                             }).ToList
    ...
    Function checkPrice(ByVal price As Decimal) As Boolean
    ...
    Function checkClassification(ByVal classification As String) As Boolean

        If classification <> "Capital" And classification <> "Selling" Then
            Return False
    ...
    Function checkCurrency(ByVal currency As String) As Boolean

        If currency <> "PHP" And currency <> "USD" And currency <> "JPY" Then
            Return False
    ...
    Function getModelCode(ByVal modelName As String) As String

        getModelCode = String.Empty

        Dim getCode As DataRow = Slipknot.rowSelectManual("SELECT fldCode FROM tblModelList WHERE fldModelName  = '" & modelName & "' LIMIT 1")

        If getCode IsNot Nothing Then
            getModelCode = Convert.ToString(getCode.Item("fldCode"))
        End If

        Return getModelCode
    End Function

    Function checkNG(ByVal itemCodeOk As Boolean, ByVal priceOk As Boolean, ByVal classOk As Boolean, ByVal currencyOk As Boolean) As String

        If itemCodeOk = False Or priceOk = False Or classOk = False Or currencyOk = False Then
            Return "NG"
    ...
    Private Sub gvData_CellFormatting(sender As Object, e As CellFormattingEventArgs) Handles gvData.CellFormatting
        ...
        Else
            ...
            If Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldItemCodeOk").Value) = False And Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldPriceOk").Value) = False And Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldClassOk").Value) = False And Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldCurrencyOk").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Item code not found in list, Invalid Price, Invalid Classification, Invalid Currency"
            ElseIf Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldItemCodeOk").Value) = False And Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldPriceOk").Value) = False And Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldClassOk").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Item code not found in list, Invalid Price, Invalid Classification"
            ElseIf Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldItemCodeOk").Value) = False And Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldPriceOk").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Item code not found in list, Invalid Price"
            ...
            ElseIf Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldPriceOk").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Invalid Price"
            ElseIf Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldClassOk").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Invalid Classification"
            ElseIf Convert.ToBoolean(e.CellElement.RowInfo.Cells("fldCurrencyOk").Value) = False Then
                e.CellElement.RowInfo.Cells("fldWarning").Value = "Invalid Currency"
            Else
    ...
    Private Sub bwSave_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bwSave.DoWork
        ...
        For counter As Integer = 0 To gvData.Rows.Count
            ...
            If Convert.ToString(gvData.Rows(counter).Cells("STATUS").Value) <> "NG" Then
                ...
                classificaiton = Convert.ToString(gvData.Rows(counter).Cells("fldClassification").Value)
                currency = Convert.ToString(gvData.Rows(counter).Cells("fldCurrency").Value)
                ...
                Slipknot.addParameter("parClassification", classificaiton.TrimStart.TrimEnd)
                Slipknot.addParameter("parCurrency", currency.TrimStart.TrimEnd)
