...
Public Class FrmProdOutputGroupProc
    ...
    Private Sub cbbAdd_Click(sender As Object, e As EventArgs) Handles cbbAdd.Click
        ...
        If Slipknot.IsFormOpen(FrmAddProdMainProcess) Then
            FrmAddProdMainProcess.Focus()
        Else

            FrmAddProdMainProcess.txtMainID.Text = gvData.CurrentRow.Cells("fldMainId").Value
            FrmAddProdMainProcess.Show(Me)
        End If
    ...
    Public Sub FetchProdOutputGroup()

        Slipknot.addParameter("MainID", globalVariables.MainID)
        Dim dtProdOutoutSub As DataTable = Slipknot.dbSelect("SP_SelectProdOutputSubProcessbyMainID")
        ' gvData.DataSource = Slipknot.dbSelect("SP_SelectProdOutputSubProcessbyMainID")

        If dtProdOutoutSub.Rows.Count = 0 Then
            Slipknot.addParameter("ID", globalVariables.MainID)
            Slipknot.addParameter("AddedBy", Convert.ToString(currentUser.Item("fldUsername")))
            Slipknot.addParameter("FGNo", globalVariables.FGNoSelected)
            Slipknot.dbAED("SP_InsertProdOutputSubProcessAuto")

            Slipknot.addParameter("MainID", globalVariables.MainID)
            gvData.DataSource = Slipknot.dbSelect("SP_SelectProdOutputSubProcessbyMainID")
        Else
            gvData.DataSource = dtProdOutoutSub
        End If
    End Sub
    ...
    Private Sub cbHPO_Click(sender As Object, e As EventArgs) Handles cbHPO.Click
        If (gvData.Rows.Count >= 1) Then
            If (gvData.CurrentRow.Cells("fldProcessCode").Value.ToString = "P032" Or gvData.CurrentRow.Cells("fldProcessCode").Value.ToString = "P033") Then
                If Slipknot.IsFormOpen(FrmHourlyProdMnt) Then
                    FrmHourlyProdMnt.Focus()
                Else
                    FrmHourlyProdMnt.mcbTitle.Text = gvData.CurrentRow.Cells("fldTitle").Value.ToString
                    FrmHourlyProdMnt.mcbProcess.Text = gvData.CurrentRow.Cells("fldMainProcess").Value.ToString
                ...
                End If
            Else
                RadMessageBox.Show(gvData.CurrentRow.Cells("fldMainProcess").Value.ToString + " is not allowed in this module", "Warning", MessageBoxButtons.OK, RadMessageIcon.Exclamation)
            End If
    ...
    Private Sub cbRMWorkProcess_Click(sender As Object, e As EventArgs) Handles cbRMWorkProcess.Click
        ...
        If (gvData.Rows.Count >= 1) Then
            If (gvData.CurrentRow.Cells("fldProcessCode").Value.ToString = "P032" Or gvData.CurrentRow.Cells("fldProcessCode").Value.ToString = "P033") Then
                ...
                Else
                    FrmRMWorkProcess.txtPCode.Text = gvData.CurrentRow.Cells("fldProcessCode").Value
                    FrmRMWorkProcess.txtItemCode.Text = gvData.CurrentRow.Cells("fldItemCode").Value
    ...
    Private Sub cbbEdit_Click(sender As Object, e As EventArgs) Handles cbbEdit.Click
        If (gvData.Rows.Count >= 1) Then
            ...
            Else
                ...
                frmAddProdOutputGroupProc.mcbItemCode.Text = gvData.CurrentRow.Cells("fldItemCode").Value
                frmAddProdOutputGroupProc.txtPartName.Text = gvData.CurrentRow.Cells("fldPartName").Value
                frmAddProdOutputGroupProc.txtPOSPID.Text = gvData.CurrentRow.Cells("fldPOSPID").Value
                frmAddProdOutputGroupProc.txtMainID.Text = gvData.CurrentRow.Cells("fldMainId").Value

                frmAddProdOutputGroupProc.mcbShift.Text = globalVariables.ShiftSelect
